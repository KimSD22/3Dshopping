<template>
    <div class="cart">
        <div class="cart-header">
            <div class="cart-header-title">장바구니 목록</div>
            <div class="cart-header-main">
                <div class="cart-info">상품정보</div>
                <div class="cart-price">단가</div>
                <div class="cart-count">갯수</div>
                <div class="cart-cost">합계</div>
                <div class="cart-delete">삭제</div>
            </div>
        </div>
            ...
            <div class="cart-content-main" v-for="(item, index) in cartList">
                ...
                <div class="cart-delete">
                    <span class="cart-control-delete"
                          @click="handleDelete(index)">삭제하기</span>
                </div>
            </div>
            <div class="cart-empty" v-if="!cartList.length">장바구니가 비어있습니다</div>
        </div>
        <div class="cart-promotion" v-show="cartList.length">
            <span>쿠폰사용</span>
            <input type="text" v-model="promotionCode">
            <span class="cart-control-promotion"
                  @click="handleCheckCode">인증</span>
        </div>
        <div class="cart-footer" v-show="cartList.length">
            <div class="cart-footer-desc">
                총개수 <span>{{countAll}}</span>
            </div>
            <div class="cart-footer-desc">
                결제금액 <span>$ {{costAll - promotion}}</span>
                <br>
        ...
        methods: {
            ...
            handleCheckCode(){
                if(this.promotionCode === ''){
                    window.alert('코드를 입력해 주세요');
                    return;
                }
                if(this.promotionCode !== 'ccylovehs'){
                    window.alert('코드가 존재하지 않습니다');
                    return;
                }
                this.promotion = 500;
            },
